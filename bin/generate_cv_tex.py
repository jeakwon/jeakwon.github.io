...
\titlespacing*{\subsection}{0pt}{0.45em}{0.3em}
...
def _render_entry(entry: dict) -> list:
    ...
    head = "\\item[\\textcolor{accent}{\\checkmark}]" if is_selected else "\\item[]"
...
def render_publication_summary(entries: list, stats: dict | None) -> str:
    ...
    if stats and "stats" in stats:
        ...
        cite_line = (
            f"\\textbf{{Citations:}} {s.get('citations', '?')} "
            f"$\\cdot$ h-index {s.get('h_index', '?')} "
            f"$\\cdot$ i10-index {s.get('i10_index', '?')}"
        )
        if date:
...
def render_cv_section_items(section: dict) -> str:
    ...
    itemize_open = "\\begin{itemize}[itemsep=0.25em]" if stacked else "\\begin{itemize}"
    lines = [f"\\block{{{title}}}", "", itemize_open]
    ...
    for item in section.get("contents", []):
        ...
        if item_title and institution:
            if stacked:
                indent = "\\hspace*{1em}"
                line = (
                    f"\\textbf{{{escape_latex(item_title)}}} \\hfill {escape_latex(year)}\\\\\n"
                    f"{indent}{escape_latex(institution)}"
                )
                descs = item.get("description", [])
                for d in descs:
                    if isinstance(d, str):
                        line += f"\\\\\n{indent}$\\diamond$~{escape_latex(d)}"
                ...
                for d in descs:
                    ...
                    if escaped.startswith("("):
                        ...
                        plain_descs.append(escaped)
                desc_parts = []
                if plain_descs:
                    desc_parts.append(f"\\textit{{{'; '.join(plain_descs)}}}")
                if meta_descs:
                    desc_parts.append(
                        f"{{\\small \\textcolor{{gray}}{{{' '.join(meta_descs)}}}}}"
                    )
                if desc_parts:
...
def render_research_areas(section: dict) -> str:
    """Render research areas as a descriptive paragraph with bold area names and pub tags."""
    title = escape_latex(section.get("title", "").lower())
    area_parts = []
    for area in section.get("contents", []):
        area_name = escape_latex(area.get("area", ""))
        sub_parts = []
        for sub in area.get("subs", []):
            name = escape_latex(sub.get("name", ""))
            pubs = sub.get("pubs", "")
            if pubs:
                sub_parts.append(
                    f"{name} ({{\\small \\textcolor{{gray}}{{{escape_latex(pubs)}}}}})"
                )
            else:
                sub_parts.append(name)
        area_parts.append(f"\\textbf{{{area_name}}}: " + " and ".join(sub_parts))
...
def render_research_interest(section: dict) -> str:
    ...
    if refs:
        ...
        for item in refs:
            ...
            role = item.get("role", "")
            role_tex = (
                f" {{\\color{{accent}}({render_inline(str(role))})}}" if role else ""
            )
            lines.append(
                f"\\item \\textit{{{paper_title}}}, \\textbf{{{venue}}}, {authors}{role_tex}"
            )
...
def generate_cv_tex():
    ...
    # Extract Position + Affiliation for header tagline
    position = ""
    affiliation = ""
    for section in cv_data:
        if section.get("title") == "General Information":
            for item in section.get("contents", []):
                if item.get("name") == "Position":
                    position = item.get("value", "")
                if item.get("name") == "Affiliation":
                    affiliation = item.get("value", "")
            break

    # Short form for header inline
    position_short = position.replace("Postdoctoral Researcher", "Postdoc")
    aff_match = re.search(r"\(([^)]+)\)", affiliation)
    affiliation_short = aff_match.group(1) if aff_match else affiliation

    header_block = (
        "\\noindent "
        f"{{\\LARGE\\scshape\\color{{accent}} {escape_latex(name.title())}}}"
        "\\hspace{0.8em}"
        f"{{\\small\\scshape\\color{{gray}} {escape_latex(position_short)} @ {escape_latex(affiliation_short)}}}"
        "\\\\[0.3em]\n"
        "\\noindent {\\small "
        f"\\href{{mailto:{EMAIL}}}{{{EMAIL}}} $\\cdot$ "
        f"\\href{{{WEBSITE_URL}}}{{Personal Page}} $\\cdot$ "
        "\\href{https://github.com/jeakwon}{GitHub} $\\cdot$ "
        f"\\href{{{SCHOLAR_URL}}}{{Scholar}} $\\cdot$ "
        "\\href{https://linkedin.com/in/jeakwon}{LinkedIn}"
        "}\n"
        "\\vspace{0.3em}\n"
    )

    # Build document
    parts = [PREAMBLE, footer, "\\begin{document}", "", header_block]
    ...
    for section in cv_data:
        ...
        if st == "time_table":
            ...
            parts.append(render_research_areas(section))
        elif st == "paragraph":
            parts.append(render_paragraph(section))
        elif st == "role_suffix":
